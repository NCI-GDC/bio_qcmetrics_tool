...
class ExportStarStats(ExportQcModule):
    ...
    def __add_arguments__(cls, subparser):
        subparser.add_argument('--final_log_inputs', action='append', 
            help='Input star log file.') 

        subparser.add_argument('--gene_counts_inputs', action='append',
            help='Input star gene counts file.') 

        subparser.add_argument('-j', '--job_uuid', type=str, required=True,
            help='The job uuid associated with the inputs.')

        subparser.add_argument('--bam', action='append', 
    ...
    def do_work(self):
        ...
        if not self.options['final_log_inputs'] and not self.options['gene_counts_inputs']:
            msg = "You must provide at least one --final_log_inputs or --gene_counts_inputs parameter"
            self.logger.error(msg)
            raise Exception(msg)

        self.logger.info('Processing {0} STAR log files...'.format(
            len(self.options['final_log_inputs'])))

        self.logger.info('Processing {0} STAR gene count files...'.format(
            len(self.options['gene_counts_inputs'])))

        if self.options['final_log_inputs']:
            for star_file, bam in zip(self.options['final_log_inputs'], self.options['bam']):
        ...
        if self.options['gene_counts_inputs']:
            for star_file, bam in zip(self.options['gene_counts_inputs'], self.options['bam']):
                source = os.path.basename(star_file)
                assert source not in self.data, "Duplicate input files?? {0}".format(source)
                self.logger.info("Processing {0}".format(source))
                self.data[source] = dict() 
                self.data[source]['star_gene_counts'] = \
        ...
        try:
            ...
            try:
                parsed_data['unmapped_mismatches'] = int(round(
                    unmapped_count * (parsed_data['unmapped_mismatches_percent'] / \
                                      total_unmapped_percent), 0))
                parsed_data['unmapped_tooshort'] = int(round(unmapped_count * \
                    (parsed_data['unmapped_tooshort_percent'] / total_unmapped_percent), 0))
                parsed_data['unmapped_other'] = int(round(unmapped_count * \
                    (parsed_data['unmapped_other_percent'] / total_unmapped_percent), 0))
        ...
        for source in sorted(self.data):
            ...
            if 'star_gene_counts' in self.data[source]:
                ...
                for strand in self.data[source]['star_gene_counts']:
                    if strand == 'bam': continue
                    for key in sorted(self.data[source]['star_gene_counts'][strand]):
                        row = dict()
                        row['category'] = key
                        row['value'] = self.data[source]['star_gene_counts'][strand][key]
                        row['strand'] = strand
